...
final class WindowMover {
    ...
    static func currentGap(respectSystemMargins: Bool) -> CGFloat {
        guard respectSystemMargins else { return 0 }
        let defaults = UserDefaults(suiteName: "com.apple.WindowManager")
        let marginsEnabled =
            (defaults?.object(forKey: "EnableTiledWindowMargins") as? NSNumber)
            .map { $0.boolValue } ?? true
        guard marginsEnabled else { return 0 }
        let spacing = (defaults?.object(forKey: "TiledWindowSpacing") as? NSNumber)?.doubleValue ?? 8
        return min(max(CGFloat(spacing), 0), 64)
    }
